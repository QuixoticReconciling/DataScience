...
- 12.63907. Unsure if this is hours or minutes.
...
- Which carriers differ in the two lists?
  - EV and YV are on the mean list but not the median list, and UA and VX are on the median list but not the mean list
- What do negative values of `dep_delay` mean?
  - Negative values mean the flight leave early.
...
test_iqr <-
  quantile(v_test_iqr, 0.75) - quantile(v_test_iqr, 0.25)
test_iqr
```
...
- Which carrier has the biggest difference between its `sd` and `IQR`? What might this mean?
  - HA has a really big difference between its standard deviation and IQR. This could be because HA has some really large delay values that are far away from the mean.
...
- For what values of `slope` is the correlation positive?
  - The correlation is positive when the slope is positive.
- For what values of `slope` is the correlation negative?
  - The correlation is negative when the slope is negative.
...
- What do you predict the correlations to be?
  - I predicted a 0 for pearson correlation and to be 1 for spearman correlation
- What are the actual correlations?
  - Both correlations are actually near 0
