...
df_q3 <- 
  flights %>% 
  filter(arr_time != sched_arr_time & abs(arr_time - sched_arr_time) <= 10
  )
...
This code doesn't quite what the user intended---they were trying to filter for only those flights where the destination was Boston airport. What went wrong?
...
df_q6a <- flights %>%
  filter(dest == "JFK" | dest == "EWR" | dest == "LGA")
df_q6a
...
- Was this dataset assembled to study flights *out of* NYC, or *into* NYC? How do you know?
  - This dataset was assembled to study flights out of NYC.
  - There are no data points with flights with NYC as the destination
