...
- Which `class` has the most vehicles? SUVs
- Which `class` has the broadest distribution of `cty` values? Two seater
- Which graph---`vis1` or `vis2`---best helps you answer each question? graph one uses frequency polygons and are better for showing the relative size of each group so size one is better for the first question. Graph two uses a density plot and its better for showing where the data is located. This means that it's better for answering the second question.
...
```{r q4-task}
## TODO: Run this chunk; play with different bin widths
diamonds %>%
  filter(carat < 1.1) %>%
...
**Observations**
- There's more at the round number amounts.
